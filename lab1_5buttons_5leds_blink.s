...
; ------------------- Configurações do microcontrolador -------------
    ...
    movwf   TRISA   
    movlw   0x1F    ; PORTD com os pinos 0-4 como entrada (botões) e 5-7 como saída (LEDs)
    movwf   TRISD
    movlw   0xFC    ; PORTC com pinos 0 e 1 como saída (LEDs)
    movwf   TRISC
...
; ------------------- Programa principal -------------
Loop:
    ; Verifica o estado de cada botão e faz o LED piscar quando o botão for pressionado

    ; Botão 1
    BTFSS   BOTAO1      ; Se BOTAO1 está em nível alto (não pressionado), pula
    goto    Blink_LED1  ; Se BOTAO1 está em nível baixo (pressionado), vai para Blink_LED1
...
Blink_LED1:
    ...
    LED1_OFF
    call    DELAY
    BTFSS   BOTAO1      ; Verifica se o botão ainda está pressionado
    goto    Blink_LED1  ; Se estiver pressionado, continua piscando
    goto    Loop        ; Se não, volta ao loop principal
